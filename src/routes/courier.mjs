...
router.get('/current-orders', ensureAuthenticated, async (req, res) => {
  try {
    const courierId = req.user._id;
    if (!courierId) {
      return res.status(401).send('Не авторизован');
...
router.get('/current-orders/:orderId', ensureAuthenticated, async (req, res) => {
  try {
...
router.get('/completed-orders', ensureAuthenticated, async (req, res) => {
  try {
    const courierId = req.user._id;
    if (!courierId) {
      return res.status(401).send('Не авторизован');
...
router.get('/canceled-orders', ensureAuthenticated, async (req, res) => {
  try {
    const courierId = req.user._id;
    if (!courierId) {
      return res.status(401).send('Не авторизован');
...
router.get('/courier-info', ensureAuthenticated, (req, res) => {
  res.render('partials/user/info', { layout: false, user: req.user });
});

router.post('/courier-info/edit', ensureAuthenticated, async (req, res) => {
